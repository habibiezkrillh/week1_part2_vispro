...
void main() {
  ...
  String message = 'Nilai maksimum yang bisa dibawa: $maxNilai' +
      (totalBerat > kapasitas
          ? ', Kelebihan berat: ${totalBerat - kapasitas} kg'
          : '');

  // Menampilkan pesan sesuai dengan total berat
  stdout.write(totalBerat <= kapasitas
      ? message
      : 'Kagak muat lagi lah, maksimal kapasitas $kapasitas kg!');
...
void displayItems(
    List<String> barang, List<int> berat, List<int> nilai, int index) {
  index < barang.length
      ? (() {
          stdout.write(
              '${index + 1}. ${barang[index]} (Berat: ${berat[index]} kg, Nilai: ${nilai[index]})');
          displayItems(
              barang, berat, nilai, index + 1); // rekursi untuk item berikutnya
        })()
...
int getUserInput(List<String> barang, List<int> berat, int index) {
  return index < barang.length
      ? (() {
          ...
          return (jumlah * berat[index]) +
              getUserInput(
                  barang, berat, index + 1); // rekursi untuk total berat
        })()
      : 0; // mengembalikan 0 jika sudah sampai akhir daftar
}

// Fungsi untuk menghitung nilai maksimum yang bisa dibawa
int getMaxValue(List<String> barang, List<int> berat, List<int> nilai,
    int kapasitas, int index, int currentValue) {
  return index >= barang.length || kapasitas <= 0
      ? currentValue // jika sudah tidak ada barang atau kapasitas habis
      : (berat[index] <= kapasitas
          ? getMaxValue(barang, berat, nilai, kapasitas - berat[index],
              index + 1, currentValue + nilai[index])
          : getMaxValue(
              barang, berat, nilai, kapasitas, index + 1, currentValue));
}

// Fungsi untuk menampilkan barang-barang yang dibawa
void displayBarangDibawa(int totalBerat, List<String> barang, List<int> berat,
    int kapasitas, int index) {
...
List<String> findItems(
    List<String> barang, List<int> berat, int kapasitas, int index) {
  return index < barang.length && kapasitas > 0
      ? (berat[index] <= kapasitas
          ? [barang[index]] +
              findItems(barang, berat, kapasitas - berat[index], index + 1)
...
void displayItemsDibawa(List<String> barangDibawa, int index) {
  index < barangDibawa.length
      ? (() {
          ...
          displayItemsDibawa(
              barangDibawa, index + 1); // rekursi untuk item berikutnya
        })()
      : null; // Menggunakan ternary operator
}
